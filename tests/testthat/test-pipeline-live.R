# ==============================================================================
# LIVE Pipeline Tests for vtschooldata
# ==============================================================================
#
...
test_that("Vermont DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Vermont DOE URL and verify HTTP 200")
})
...
test_that("Can download Vermont enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Vermont enrollment file", {
  skip_if_offline()
...
test_that("vtschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- vtschooldata::get_available_years()
  ...
  tryCatch({
    raw <- vtschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- vtschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- vtschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- vtschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- vtschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- vtschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- vtschooldata:::get_cache_path(2024, "enrollment")
